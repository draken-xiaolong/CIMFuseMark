...
三个人工基线在相同数据、规范化和攻击结果上运行：RadialHist使用128-bin半径直方图；SphericalHist拼接半径、方位角和仰角直方图；MomentFusion融合半径、绝对坐标分位数及协方差谱。它们代表主流人工几何统计思路的统一适配，不宣称逐字复现原论文。

\section{结果与分析}
\subsection{攻击强度--NC曲线}
\begin{figure}[H]\centering\includegraphics[width=\textwidth]{figures/robustness_grid.pdf}\caption{地域隔离测试集九类攻击的强度--NC曲线。实线为平均NC，虚线为$q_{0.05}$。}\label{fig:robust}\end{figure}
...
\begin{table}[H]\centering\caption{相同地域隔离测试集和攻击实现下的基线比较。}\label{tab:baseline}\input{baseline_table.tex}\end{table}
