...
def main():
    ...
    total,done,missing,json_done,payload_done,size=db.execute("select count(*),sum(status='done'),sum(status='missing'),sum(type='json' and status='done'),sum(type='payload' and status='done'),coalesce(sum(size),0) from urls").fetchone()
    shards=sorted(glob.glob(str(root/'payload_*.zip')))
    result={'total':total,'done':done or 0,'missing':missing or 0,'queued':total-(done or 0)-(missing or 0),'progress_percent':round(100*((done or 0)+(missing or 0))/max(total,1),3),'json_done':json_done or 0,'payload_done':payload_done or 0,'content_gib':round(size/2**30,3),'zip_shards':len(shards),'closed_shards':max(0,len(shards)-1),'allocated_gib':round(sum(os.path.getsize(x) for x in shards)/2**30,3),'complete':total==(done or 0)+(missing or 0)}
