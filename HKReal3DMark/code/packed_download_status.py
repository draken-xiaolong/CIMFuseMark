...
def main():
    ...
    total,done,missing,json_done,payload_done,json_queued,payload_queued,size=db.execute("select count(*),sum(status='done'),sum(status='missing'),sum(type='json' and status='done'),sum(type='payload' and status='done'),sum(type='json' and status='queued'),sum(type='payload' and status='queued'),coalesce(sum(size),0) from urls").fetchone()
