...
def tensors(graph, relations, device, relation_mode, feature_mode, seed):
...
def main() -> None:
    ...
    input_dim = (0 if feature_mode == "lgfm" else
                 graph_tensors(next(iter(graphs.values())), relations, device, relation_mode,
    ...
    with torch.no_grad():
        ...
        if background:
            background_embeddings = torch.stack([
                model.encode(*tensors(build_citygml_graph((DATA_ROOT / item["path"]).resolve()),
